...
function AntMedia() {
  ...
  function screenShareOffNotification() {
    antmedia.handleSendNotificationEvent('SCREEN_SHARED_OFF', myLocalData.streamId);
  ...
  function screenShareOnNotification() {
    console.log('screenShareOnNotificationscreenShareOnNotificationscreenShareOnNotification');
    ...
    antmedia.handleSendNotificationEvent('SCREEN_SHARED_ON', myLocalData.streamId);

    setPinnedVideoId('localVideo');
  }
  function handleScreenshareNotFromPlatform() {
    setIsScreenShared(false);
    if (cam.find(c => c.eventStreamId === 'localVideo' && c.isCameraOn === false)) {
  ...
  function handleSendMessage(message) {
    if (myLocalData.streamId) {
      ...
      if (iceState !== null && iceState !== 'failed' && iceState !== 'disconnected') {
        console.log('handleSendMessagehandleSendMessagehandleSendMessagehandleSendMessage', message, myLocalData);
    ...
    if (notificationEvent != null && typeof notificationEvent == 'object') {
      ...
      if (eventType === 'CAM_TURNED_OFF') {
        ...
        if (participants.find(p => p.id === eventStreamId)) {
          ...
          antmedia.assignVideoTrack(participants.find(p => p.id === eventStreamId).videoLabel, id, false);
        }
        ...
        console.log('PIN_USERPIN_USERPIN_USERPIN_USERPIN_USERPIN_USERPIN_USERPIN_USER', notificationEvent, eventStreamId, screenSharedVideoId);
        if (notificationEvent.streamId === myLocalData.streamId && !isScreenShared) {
          let requestedMediaConstraints = {
            width: 640,
            height: 480,
          };
  ...
  function handlePublish(publishStreamId, token, subscriberId, subscriberCode) {
    antmedia.publish(publishStreamId, token, subscriberId, subscriberCode, streamName, roomName, '{someKey:somveValue}');
  }
  function handlePlayVideo(obj, publishStreamId) {
    console.log('handlePlayVideohandlePlayVideohandlePlayVideohandlePlayVideo', obj);
      ...
      <Grid container className="App-header" justifyContent="center" alignItems={'center'}>
        ...
        >
          <SnackbarProvider
            ...
            maxSnack={3}
            content={(key, notificationData) => <AntSnackBar id={key} notificationData={notificationData} />}
          >
            {waitingOrMeetingRoom === 'waiting' ? (
              <WaitingRoom streamName={streamName} handleStreamName={name => setStreamName(name)} handleChangeRoomStatus={status => setWaitingOrMeetingRoom(status)} />
            ) : (
